...
### Cookie Missing After Login (Secure Flag Issue)

**Symptom:** The OAuth2 flow fails when returning from the IDP. The server logs show the `oauth2_auth_request` cookie is successfully generated prior to redirecting to the IDP, but is completely missing (null) when the browser returns.

**Cause:** Modern web browsers enforce strict security policies around cookies. If a backend server sends a `Set-Cookie` header with the `Secure` flag set to `true`, but the request is occurring over an unencrypted `http://` connection (such as local development on `http://localhost` or `http://sso-peanut.localhost`), the browser will silently drop the cookie. When the user completes the flow at the IDP and redirects back, the browser vault is empty, and the backend cannot restore the authorization context.

**Solution:** The `Secure` flag must be set dynamically based on the incoming request environment. In this prototype, we updated `CookieUtils.java` to evaluate the incoming request protocol:
```java
cookie.setSecure(request.isSecure());
```
This allows cookies to flow freely during local HTTP development. In production, as long as the API Gateway appropriately sets the `X-Forwarded-Proto: https` header, Spring Boot will correctly evaluate `request.isSecure() == true` and protect the cookie.
